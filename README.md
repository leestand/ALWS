...
> **"시민과 점주가 모두 상생할 수 있는 정책을 위해, 모두가 이기는(All Wins 😄) 착한가격업소 대시보드"**
...
👉 [ALWS 서비스 발표자료 보러가기](./LeeseoAn_Portfolio_ALWS.pdf)
...
광진구 지역의 착한가격업소 현황을 시각화한 인터랙티브 대시보드입니다.  
월별 업종별 평균가격, 유동인구, 매출 추정 데이터 등을 한눈에 확인할 수 있습니다.
...
ALWS
├── data/                          
│   └── price_crawl.ipynb          # 지역별 가격 데이터 수집 코드
├── analysis/                     
│   ├── business_closure_rate.ipynb     # 폐업률 분석
│   ├── segment.ipynb                   # 지역 상권 세분화 분석
│   └── Sales_per_living population.sql # 생활인구당 매출 SQL 쿼리
├── dashboard/                    
│   └── dashboard_sample.jpg      # Tableau 시각화 결과 샘플 이미지
├── LeeseoAn_Portfolio_ALWS.pdf    # ALWS 포트폴리오 PDF
